...
/*
   ...
   22.june 1999 - Added GetCount()
	...
	4. dec. 99	 - Added SortEx, the previous Sort function did not sort RootItems.
				 ...
				 *	
				 * Today it磗 actually one year ago this code went public (good or bad you decide). 
                 * 

	8. dec. 99   - Added Arne Schild磗 improvements, see his improvements in the comment section on the website.
				 ...
				 - UpdateData now has a default parametre(BOOL bUpdateRow), which forces the listview item to be written.
				   this means you don磘 have to call UpdateData and CListCtrl::SetItemText afterwards when doing dynamic updates.
	...
	WHAT'S UP :
				- NEW RELEASES can in future be found at www.codeproject.com, don磘 forget to subscribe.
				- Peace on earth. 
...
/////////////////////////////////////////////////////////////////////////////
int CSuperGridCtrl::m_SortSubItem=0;
BOOL CSuperGridCtrl::m_SortSubAscending=0;
...
BEGIN_MESSAGE_MAP(CSuperGridCtrl, CToolTipListCtrl)
...
CTreeItem*  CSuperGridCtrl::InsertRootItem(CItemInfo * lpInfo)
{
	...
	lvItem.iIndent = 1;
	lvItem.iSubItem = 0;
	CToolTipListCtrl::InsertItem(&lvItem);
	SetCheck(lvItem.iItem, lpInfo->GetCheck());//20160313
...
void CSuperGridCtrl::SetCheckAll(CTreeItem *pItem, BOOL bState)
{
	if( (pItem->m_pParent==NULL) || (pItem->m_pParent!=NULL)&&(!IsCollapsed(pItem->m_pParent)) )
		SetCheck(GetCurIndex(pItem), bState);
	CItemInfo *pInfo = GetData(pItem);
	pInfo->SetCheck(bState);

	POSITION pos = pItem->m_listChild.GetHeadPosition();
	for (int i=0; pos; i++)
	{
		CTreeItem* pChildItem = (CTreeItem*)pItem->m_listChild.GetAt(pos);

		if(ItemHasChildren(pChildItem))
			SetCheckAll(pChildItem, bState);
		else
		{
			if(!IsCollapsed(pItem))
				SetCheck(GetCurIndex(pChildItem), bState);
			CItemInfo *pInfo = GetData(pChildItem);
			pInfo->SetCheck(bState);
			TRACE("%d  %s\n",IsCollapsed(pItem), pInfo->GetItemText());
		}
...
void CSuperGridCtrl::SetCheckAll(BOOL bState)
{
	if (m_RootItems.GetCount() > 0)
	{
...
{
	//If not do so,when an item's parent is collapsed, you call GetCurIndex will not return -1,this sometimes can make an error.
	if(!IsCollapsed(pItem))
	{
		if(ItemHasChildren(pItem))
		{
			...
			while (pos != NULL)
			{
				CTreeItem * pNext= (CTreeItem * )(pItem->m_listChild.GetNext(pos));
				pNext->m_nIndex = -1;
				HideChildren(pNext,bHide,nItem+1);
				DeleteItem(nItem);
			}
		}
	}
...
void CSuperGridCtrl::Sort(CTreeItem* pParent, int SubItem, BOOL bAscending, BOOL bSortChildren)
{
	...
	if (nChildren > 1)
	{
		...
		SortItem(ppSortArray, nChildren, SubItem, bAscending);
	...
	if(bSortChildren)
	{
		...
		while (pos)
		{
			...
			Sort(pChild, SubItem, bAscending, TRUE);
		}
	}
}

void CSuperGridCtrl::SortEx(int SubItem, BOOL bAscending, BOOL bSortChildren)
{
	//CMFCListCtrl 定义
	//m_iSortedColumn = SubItem;
	...
	if (nItems > 0)
	{
		...
		if(bSortChildren)
		{
			...
			while(posSortChildren != NULL)
			{
					...
					Sort(pParent, SubItem, bAscending, TRUE);//sort children			
			}	
		}	
		...
		DeleteAllItems();//this is quite okay, I don磘 delete the internal state

		//排序Root sort rootitems
...
int CSuperGridCtrl::CompareChildren(const void* p1, const void* p2)
{
	...
	int cm=0;
	if(m_SortSubItem==0)
		cm = pItem1->GetItemText().Compare(pItem2->GetItemText());
	else
		cm = pItem1->GetSubItem(m_SortSubItem-1).Compare(pItem2->GetSubItem(m_SortSubItem-1)); //-1 CSuperGridCtrl 多少一个subitem
...
void CSuperGridCtrl::MakeColumnVisible(int nCol)
{
	...
	CMFCHeaderCtrl& pHeader = GetHeaderCtrl();
...
BOOL CSuperGridCtrl::SetSubItemImageList(CImageList *pImageList)
{
	if(pImageList!=NULL)
	{
...
BOOL CSuperGridCtrl::PreCreateWindow(CREATESTRUCT& cs) 
{
	...
	return CToolTipListCtrl::PreCreateWindow(cs);
}
...
void CSuperGridCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if(lpDrawItemStruct->itemAction == ODA_DRAWENTIRE)
	{
		...
		//得到当前图标 you have overridden this I hope :-)
		...
		int nOffset = ( (lpDrawItemStruct->rcItem.bottom-lpDrawItemStruct->rcItem.top) - m_cyImage)/2;
		int nY = lpDrawItemStruct->rcItem.bottom - m_cyImage - nOffset;
		...
		{
			int RowLeft = rc.left;
			if( GetItemState(lpDrawItemStruct->itemID, LVIF_STATE) & LVIS_DROPHILITED)//if dragging show a SelectDropTarget alike effect :)
				uiFlags |= ILD_BLEND50;
		
			if((nWidth=m_cxImage-nWidth) >-1)//calc width of icon
			{
				DrawTreeItem(pDC, pSelItem, lpDrawItemStruct->itemID, rc);
				RowLeft+=GetIndent(pSelItem) * m_cxImage;
				...
				ImageList_DrawEx(m_himl, lvi.iImage, pDC->m_hDC,  
									...
									CLR_DEFAULT, 
									uiFlags);
				RowLeft+=m_cxImage+OFFSET_FIRST;
			}
			...
			if (nStateImageMask)
			{
				...
				if (pImageList)
				{
					...
					{
						RowLeft = RowLeft+(m_cxImage-cxIcon)/2;
						pImageList->Draw(pDC, nImage, CPoint(RowLeft, nY), ILD_TRANSPARENT);
						RowLeft+=m_cxImage+OFFSET_FIRST;
					}
				}
			}
		...
		//画子项
		{
			...
			for(int nColumn=1; GetColumn(nColumn, &lvc); nColumn++)
			{
				...
				if (rcItem.left < rcClipBox.right && rcItem.right > rcClipBox.left && rcItem.right > rcItem.left)
				{
					...
					if(iSubImage!=-1)
					{
						...
						if(himl!=NULL)
						{
							...
							CRect rect;
							GetSubItemRect(lpDrawItemStruct->itemID, nColumn, LVIR_BOUNDS, rect);
				...
				if (lpDrawItemStruct->itemState & ODS_SELECTED)
					DrawComboBox(pDC, pSelItem, lpDrawItemStruct->itemID, nColumn, nSubItemWidth);
			}//for
			}
	}//ODA_DRAWENTIRE
}
	...
	if(ht.iItem!=-1)
	{
		...
		if(pItem!=NULL)
		{
			if(bHit)//if checkbox
			{
				...
				bool NewCheck = !GetCheck(ht.iItem);
				SetCheckAll(pItem, NewCheck);
				return 0;
			}
			...
			if(ItemHasChildren(pItem))
			{
				...
				if(rect.HitTest(point))
				{
					...
					if(bRedraw)
					{
						...
						EnsureVisible(0, 1);  //20160315 EnsureVisible(EnsureVisible, 1); 保证展开的时候第一项显示
						return 0;
					}	
				}
			}//has kids
...
void CSuperGridCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(OnItemLButtonDown(ht))
	{
		...
		if(bSelect && ht.iItem!=-1)
		{
			...
			CToolTipListCtrl::OnLButtonDown(nFlags, point);
...
void CSuperGridCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if(m_bIsDragging == TRUE)
    {
		...
		CToolTipListCtrl::OnLButtonUp(nFlags, point);
}
				...
				if (pSelItem != NULL)
				{
					...
					if(ItemHasChildren(pSelItem))
					{
						...
						if(bRedraw)
						{
							...
							EnsureVisible(0, 1);  //20160315 EnsureVisible(nScrollIndex,1); 保证展开的时候第一项显示
						}
					}//ItemHasChildren	
				}//!=NULL
			}
...
void CSuperGridCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	CToolTipListCtrl::OnMouseMove(nFlags, point);
}


void CSuperGridCtrl::OnTimer(UINT nIDEvent) 
{
	CToolTipListCtrl::OnTimer(nIDEvent);
	if(nIDEvent==1)
	{
		if(CWnd::GetCapture()!=this)
		{
...
void CSuperGridCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	CToolTipListCtrl::OnHScroll(nSBCode, nPos, pScrollBar);
}
...
void CSuperGridCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	if( GetFocus() != this) 
		SetFocus();
	CToolTipListCtrl::OnVScroll(nSBCode, nPos, pScrollBar);
}
...
void CSuperGridCtrl::OnLvnColumnclick(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	SetSortColumn(iColumn, bAscending, FALSE);

	SortEx(iColumn, bAscending, true);
	*pResult = 0;
}

//void CSuperGridCtrl::OnSysColorChange() 
//{
//	CToolTipListCtrl::OnSysColorChange();
//	//nop nothing yet
//}
...
BOOL CSuperGridCtrl::IsSupprotDrag(void)
{
	return 1;
}

void CSuperGridCtrl::SortItem(CTreeItem** ppSortArray, int nChildren, int SubItem, BOOL bAscending)
{
